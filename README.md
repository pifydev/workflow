...
- **What the children cost is shown.** Every child message's cost and tokens go to a process-wide tally; with [`@pify/usage`](https://github.com/pifydev/usage) installed the footer shows it as ` · agents $…` beside the session's own cost, which child sessions otherwise never reach. The same per-call token figure feeds the run's `budget`.
